...
def saveProbe(case,probename,postfolder,region,keyrange,var,rename,overwrite):
    #=============================================================================#
    # Main
    #=============================================================================#
    ...
    probenameAbsPath = os.path.join(caseAbsPath,postfolder,probename,region)
    ...
    for key in varToSave:
        ...
        if rename=='':
            ...
            h5TgtFile = os.path.join(probenameAbsPath, key+'_'+rename+'.h5')
                
        
        if os.path.exists(h5TgtFile)==True and overwrite==True:
            os.remove(h5TgtFile)
            print('    Saving variable "'+key+'" in "'+h5TgtFile+'"')
            ppf.savePPlist_hdf5(ppList=keyData, hdf5file=h5TgtFile, keyrange=keyrange)

        elif os.path.exists(h5TgtFile)==False:
            print('    Saving variable "'+key+'" in "'+h5TgtFile+'"')
            ppf.savePPlist_hdf5(ppList=keyData, hdf5file=h5TgtFile, keyrange=keyrange)

        elif  os.path.exists(h5TgtFile)==True and overwrite==False:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-postfolder',
                        dest='postfolder',
                        type=str,
                        required=False,
                        default='postProcessing',
                        help='name of the postProcessing folder if not called postProcessing'
                        )
    ...
    saveProbe(args.case,args.probename,args.postfolder,args.region,args.keyrange,args.var,args.rename,args.overwrite)
