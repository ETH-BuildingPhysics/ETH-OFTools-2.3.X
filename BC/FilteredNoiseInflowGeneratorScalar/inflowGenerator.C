...
:
    ...
    uFluctFinal(p.size()),
    sFluctFiltered(p.size()),
    sFluctTemporal_old(p.size()),
    sFluctTemporal(p.size()),
    sFluctFinal(p.size()),
    origin_(vector::zero),
    mapperVP_Ptr_(NULL),
    mapperIV_Ptr_(NULL),
...
:
    ...
    sFluctFiltered(ptf.sFluctFiltered,mapper),
    sFluctTemporal_old(ptf.sFluctTemporal_old,mapper),
    sFluctTemporal(ptf.sFluctTemporal,mapper),
    sFluctFinal(ptf.sFluctFinal,mapper),
    origin_(ptf.origin_),
    mapperVP_Ptr_(NULL),
    mapperIV_Ptr_(NULL),
    mapperIP_Ptr_(NULL),
    ...
    L41_(ptf.L41_,mapper),
    L42_(ptf.L42_,mapper),
    L43_(ptf.L43_,mapper),
    L44_(ptf.L44_,mapper),
    Re14(ptf.Re14,mapper),
    Re24(ptf.Re24,mapper),
    Re34(ptf.Re34,mapper),
    Re44(ptf.Re44,mapper),
    ranGen(label(time(0))),
...
:
    ...
    sFluctFiltered(p.size(),pTraits<scalar>::zero),
    sFluctTemporal_old(p.size(),pTraits<scalar>::zero),
    sFluctTemporal(p.size(),pTraits<scalar>::zero),
    sFluctFinal(p.size(),pTraits<scalar>::zero),
    ...
    L41_(p.size(), pTraits<scalar>::zero),
    L42_(p.size(), pTraits<scalar>::zero),
    L43_(p.size(), pTraits<scalar>::zero),
    L44_(p.size(), pTraits<scalar>::zero),
    Re14(p.size(), pTraits<scalar>::zero),
    Re24(p.size(), pTraits<scalar>::zero),
    Re34(p.size(), pTraits<scalar>::zero),
    Re44(p.size(), pTraits<scalar>::zero),
    ranGen(label(time(0))),
    cleanRestart_(false),
    isInitialized_(false),
    isRestart_(false),
    correlationShape_(dict.lookupOrDefault<word>("correlationShape", "exp")),
    scalarName_(dict.lookupOrDefault<word>("scalarName", "T"))
{
...
*/
    ...
    if (dict.found("uFluctTemporal") && !cleanRestart_)
    {
        ...
        if (dict.found("uFluctTemporal"))
        {
            sFluctTemporal = scalarField("sFluctTemporal", dict, p.size());
        }
    }
...
:
    ...
    sFluctFiltered(ptf.sFluctFiltered),
    sFluctTemporal_old(ptf.sFluctTemporal_old),
    sFluctTemporal(ptf.sFluctTemporal),
    sFluctFinal(ptf.sFluctFinal),
    origin_(ptf.origin_),
    mapperVP_Ptr_(NULL),
    mapperIV_Ptr_(NULL),
    mapperIP_Ptr_(NULL),
...
void Foam::inflowGenerator::initData()
{
    ...
    virtualFilteredField_s_.setSize(NY_*NZ_,0.0);
    ...
    scalarField sRField_;
    scalarField suRField_;
    scalarField svRField_;
    scalarField swRField_;
    scalarField sRefField_;
    ...
    Pstream::scatter(sRefField_);
    Pstream::scatter(sRField_);
    Pstream::scatter(suRField_);
    Pstream::scatter(svRField_);
    Pstream::scatter(swRField_);
    ...
    sMean = mapperIP_Ptr_().interpolate(sRefField_);
    Re44 = mapperIP_Ptr_().interpolate(sRField_);
    Re14 = mapperIP_Ptr_().interpolate(suRField_);
    Re24 = mapperIP_Ptr_().interpolate(svRField_);
    Re34 = mapperIP_Ptr_().interpolate(swRField_);
    ...
    const scalarField L11_=Lund_.component(tensor::XX);
    const scalarField L12_=Lund_.component(tensor::YX);
    const scalarField L13_=Lund_.component(tensor::ZX);
    const scalarField L22_=Lund_.component(tensor::YY);
    const scalarField L23_=Lund_.component(tensor::ZY);
    const scalarField L33_=Lund_.component(tensor::ZZ);
    //Info << L11_ << endl;
    forAll(L41_, I)
    {
        L41_[I]=Re14[I]/L11_[I];
    }
    forAll(L42_, I)
    {
        L42_[I]=1.0/L22_[I]*(Re24[I]-L12_[I]*L41_[I]);
    }
    forAll(L43_, I)
    {
        L43_[I]=1.0/L33_[I]*(Re34[I]-L13_[I]*L41_[I]-L23_[I]*L42_[I]);
    }
    forAll(L44_, I)
    {
        L44_[I]=sqrt(Re44[I]-sqr(L41_[I])-sqr(L42_[I])-sqr(L43_[I]));
    }
    //Info << "Foam::inflowGenerator L41 to L44 created" << endl;
    mapperVP_Ptr_.reset
    (
        new pointToPointPlanarInterpolation
        (
    ...
    if ((start+size)>virtualFilteredField_s_.size())
    {
        size=virtualFilteredField_s_.size()-start;
    }
    ...
    virtualRandomField_s_.setSize((NY_+nfK_*NLyMax_u)*(NZ_+nfK_*NLzMax_u));
...
void Foam::inflowGenerator::getRandomField()
{
    if(Pstream::master())
    {
        ...
        forAll(virtualRandomField_s_, i)
        {
                virtualRandomField_s_[i] = scalar(getRandomNumber());
        }
    }
    Pstream::scatter(virtualRandomField_u_);
    Pstream::scatter(virtualRandomField_v_);
    Pstream::scatter(virtualRandomField_w_);
    Pstream::scatter(virtualRandomField_s_);
}
...
void Foam::inflowGenerator::spatialCorrScalar()
{
    ...
    List<scalarField> virtualFilteredFieldProc_;
    virtualFilteredFieldProc_.setSize(Pstream::nProcs());

    virtualFilteredFieldProc_[Pstream::myProcNo()]=SubField<scalar>(virtualFilteredField_s_,size,start);
    ...
    forAll(virtualFilteredFieldProc_[Pstream::myProcNo()],subI)
    {
        if(doSpatialCorr)
        {
            ...
            virtualFilteredFieldProc_[Pstream::myProcNo()][subI]=sumTmp_T;

        }
        else
        {
            virtualFilteredFieldProc_[Pstream::myProcNo()][subI]=scalar(getRandomNumber());
        }
    }
    ...
    virtualFilteredField_s_ = ListListOps::combine<scalarField>(virtualFilteredFieldProc_,accessOp<scalarField>());
}
...
void Foam::inflowGenerator::temporalCorrScalar()
{
    forAll(sFluctTemporal, celli)
    {
        label& NLX_u=NLxField_[celli].component(0);

        if (NLX_u>0)
            sFluctTemporal[celli] = sFluctTemporal_old[celli] * Foam::exp(-1.0/NLX_u) +
                sFluctFiltered[celli] * Foam::sqrt( 1.0 - Foam::exp(-2.0/NLX_u) );
        else
            sFluctTemporal[celli] = 0.0;

    }
    sFluctTemporal_old = sFluctTemporal;
}

void Foam::inflowGenerator::interpolFluct()
{
    uFluctFiltered = mapperVP_Ptr_().interpolate(virtualFilteredField_);
    sFluctFiltered = mapperVP_Ptr_().interpolate(virtualFilteredField_s_);
}
...
void Foam::inflowGenerator::scaleFluct()
{
    ...
    if (doLundScaling)
    {
        ...
        sFluctFinal=uFluctTemporal.component(0)*L41_+
                    uFluctTemporal.component(1)*L42_+
                    uFluctTemporal.component(2)*L43_+
                    sFluctTemporal*L44_;
...
{	
    ...
    else
    {
        ...
        sFluctTemporal_old = sFluctFiltered;
    }
}
...
void Foam::inflowGenerator::updateCoeffs()
{
    ...
    {	
        ...
        //updtaing the patch/adjoint volume
        /*
        volScalarField& tVolField = const_cast<volScalarField&>(db().objectRegistry::lookupObject<volScalarField>(scalarName_));
        forAll(patch(), facei)
            {
            // get global cell indices for cells adjacent to patch    
                label celli = patch().faceCells()[facei];
        tVolField[celli]=sFluctFiltered[celli];
        }*/
...
void Foam::inflowGenerator::write(Ostream& os) const
{
    ...
    os.writeKeyword("scalarName") << scalarName_ << token::END_STATEMENT << nl;
    ...
    if (isRestart_)
            ...
            sFluctTemporal.writeEntry("sFluctTemporal", os);
...
void Foam::inflowGenerator::rmap(const fvPatchField<vector>& ptf, const labelList& addr)
{
    ...
    sFluctTemporal.rmap(tiptf.sFluctTemporal, addr);
